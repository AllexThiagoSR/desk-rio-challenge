...
const DeactivateDistributionService = async (queueId: number | string) => {
  ...
  if (!(queue as Queue).ticketDistributionIsActive) throw new AppError("ERR_TICKET_DISTRIBUTION_IS_DEACTIVE", 409);
  ...
  (queue as Queue).ticketDistributionIsActive = false;
